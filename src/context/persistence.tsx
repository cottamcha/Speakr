...
import { isEqual as match } from "lodash-es"

type Map = {[ket: string]: any}
    ...
    const revalidate = async <T extends Map,>(
        ...
        callback?: (latestData: T) => void
            ...
            const isEqual = Object
                .keys(persistedData)
                .every((property)=> {
                    return match(lastestData[property], persistedData[property])
                })

            if(!isEqual){
                setValue(key, lastestData);

                if(!!callback){
                    callback(lastestData)
                }
            }
